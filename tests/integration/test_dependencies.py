...
def test_get_current_user_valid_token_nonexistent_user(mock_db, mock_verify_token):
    mock_verify_token.return_value = sample_user.id # valid token
    mock_db.query.return_value.filter.return_value.first.return_value = None #DB returns none for no user
