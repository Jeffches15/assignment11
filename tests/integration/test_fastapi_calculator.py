...
def test_add_api(client):
    ...
    response = client.post('/add', json={'a': 14, 'b': 5})
    
    # Assert that the response status code is 200 (OK)
    assert response.status_code == 200, f"Expected status code 200, got {response.status_code}"
    
    # Assert that the JSON response contains the correct 'result' value
    assert response.json()['result'] == 19, f"Expected result 19, got {response.json()['result']}"
...
def test_subtract_api(client):
    ...
    response = client.post('/subtract', json={'a': 19, 'b': 10})
    
    # Assert that the response status code is 200 (OK)
    assert response.status_code == 200, f"Expected status code 200, got {response.status_code}"
    
    # Assert that the JSON response contains the correct 'result' value
    assert response.json()['result'] == 9, f"Expected result 9, got {response.json()['result']}"
...
def test_multiply_api(client):
    ...
    response = client.post('/multiply', json={'a': 100, 'b': 7})
    
    # Assert that the response status code is 200 (OK)
    assert response.status_code == 200, f"Expected status code 200, got {response.status_code}"
    
    # Assert that the JSON response contains the correct 'result' value
    assert response.json()['result'] == 700, f"Expected result 700, got {response.json()['result']}"
...
def test_divide_api(client):
    ...
    response = client.post('/divide', json={'a': 24, 'b': 3})
    
    # Assert that the response status code is 200 (OK)
    assert response.status_code == 200, f"Expected status code 200, got {response.status_code}"
    
    # Assert that the JSON response contains the correct 'result' value
    # "Expected result 8" is just a literal string — not read from any file or automatic comparison tool.
