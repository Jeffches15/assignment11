...
def test_get_db_yields_session():
    gen = get_db()
    db = next(gen)  # start the generator and get the session
    assert db is not None
    assert hasattr(db, "query")  # basic check that this looks like a SQLAlchemy session

    # Close the generator (which triggers the finally block)
    with pytest.raises(StopIteration):
        next(gen)

    # We can't directly check if db.close() was called, but no exceptions means cleanup ran.